...
import com.example.appguarderia.databinding.FragmentCentroBinding
import com.example.appguarderia.ui.gallery.GalleryViewModel

class CentroFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val CentroViewModel =
            ViewModelProvider(this).get(CentroViewModel::class.java)

        _binding = FragmentCentroBinding.inflate(inflater, container, false)
        val root: View = binding.root

        val textView: TextView = binding.textCentro
        CentroViewModel.text.observe(viewLifecycleOwner) {
            textView.text = it
        }
        return root
    }
